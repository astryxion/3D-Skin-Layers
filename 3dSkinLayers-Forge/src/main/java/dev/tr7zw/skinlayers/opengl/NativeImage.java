...
public final class NativeImage implements AutoCloseable {
    ...
    public void downloadTexture(int i, boolean bl) {
        int previousPackAlignment = org.lwjgl.opengl.GL11.glGetInteger(org.lwjgl.opengl.GL11.GL_PACK_ALIGNMENT);
        this.format.setPackPixelStoreState();
        GlStateManager._getTexImage(3553, i, this.format.glFormat(), 5121, this.buffer);
        org.lwjgl.opengl.GL11.glPixelStorei(org.lwjgl.opengl.GL11.GL_PACK_ALIGNMENT, previousPackAlignment);
    ...
    public void flipY() {
        ...
        for (int y = 0; y < height / 2; y++) {
            ...
            for (int x = 0; x < width; x++) {
                int top = getPixelRGBA(x, y);
                int bottom = getPixelRGBA(x, oppositeY);
                setPixelRGBA(x, y, bottom);
                setPixelRGBA(x, oppositeY, top);
            }
        }
    }

    public static NativeImage fromBufferedImage(BufferedImage img) {
