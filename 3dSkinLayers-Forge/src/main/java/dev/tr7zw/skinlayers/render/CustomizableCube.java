...
public class CustomizableCube {
    ...
    public void render(boolean redTint) {
        ...
        for (int id = 0; id < polygonCount; id++) {
            ...
            tessellator.setNormal(polygon.normal.x, polygon.normal.y, polygon.normal.z);
            for (int i = 0; i < 4; i++) {
                Vertex vertex = polygon.vertices[i];
                if (redTint) {
                    tessellator.setColorRGBA(255, 127, 127, 255);
                }
                tessellator.addVertexWithUV(vertex.pos.x, vertex.pos.y, vertex.pos.z, vertex.u, vertex.v);
            }
        }
        tessellator.draw();
    }
