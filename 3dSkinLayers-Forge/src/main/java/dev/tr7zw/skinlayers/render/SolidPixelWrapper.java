...
public class SolidPixelWrapper {
    ...
    private static boolean hasPixel(NativeImage natImage, int u, int v) {
        if (u < 0 || u >= natImage.getWidth() || v < 0 || v >= natImage.getHeight()) {
            return false;
        }
        int pixel = natImage.getPixelRGBA(u, v);
        if (NativeImage.getA(pixel) != 0) {
            return true;
        }
        return (NativeImage.getR(pixel) | NativeImage.getG(pixel) | NativeImage.getB(pixel)) != 0;
    }
    ...
    private static void addPixel(NativeImage natImage, List<CustomizableCube> cubes, float pixelSize, boolean onBorder, int u,
        ...
        if (hasPixel(natImage, u, v)) {
            Set<Direction> hide = new HashSet<>();
            if (!onBorder) {
                for (int i = 0; i < offsets.length; i++) {
                    int tU = u + offsets[i][1];
                    int tV = v + offsets[i][0];
                    if (tU >= 0 && tU < natImage.getWidth() && tV >= 0 && tV < natImage.getHeight()
                            && hasPixel(natImage, tU, tV)) {
